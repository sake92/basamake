...
import ba.sake.basamake.index.scalasrc.ScalaHoverExtractor
import ba.sake.basamake.index.javasrc.JavaHoverExtractor
...
class HoverProvider(val workspaceIndex: WorkspaceIndex) extends StrictLogging {
  ...
  private[index] def sourceLineFallback(defn: SymbolDefinition): Option[(String, Option[String])] = {
    try {
