...
/** Thread-safe routing table mapping document URIs to owning BSP connections.
...
final class RoutingTable private (private var entries: Map[BspConnectionId, List[String]]):
  ...
  /** Remove a connection from the routing table (clean detach). */
  def remove(connId: BspConnectionId): Unit = synchronized {
    entries = entries - connId
  }

  /** Find the owning connection for a URI via longest-prefix match.
    * Returns None if no connection owns this URI. */
  ...
  /** Return all registered source directory URIs for a connection. */
