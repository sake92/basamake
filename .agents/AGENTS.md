...
- **No background indexing queue** — inline indexing is serialized per fingerprint by `SourceJarIndexer`'s `ReentrantLock` (parks virtual threads, no carrier pinning); parsing ~90 source jars concurrently used to spike committed heap past 1GB, but lookups now index one jar at a time on demand, so cold indexing peaks far lower. The JDK is the one background exception: `ensureJdkIndexed` (started at `initialize()`, unconditionally — no BSP dependency) indexes `src.zip` on its own virtual thread, and a COLD JDK is never indexed inline by a lookup — a `java.*` lookup before the background index finishes is a fast transient miss
...
- `definition()`/`references()`/`hover()`: `CompletableFuture.supplyAsync(..., navigationExecutor)` → `workspaceIndex.gotoDefinitions`/`references`/`HoverProvider` (incl. `includeDecl`) — the executor is a virtual-thread-per-task executor so inline dep indexing never blocks a common-pool or lsp4j thread
