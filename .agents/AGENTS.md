...
- **Index eagerly, unpack lazily.** `ensureIndexed`/`ensureJdkIndexed` (from `initialize()` and every BSP `dependencySources` handshake) build the LMDB symbol index in the background; source files are extracted on first lookup hit (`ensureEntryExtracted`), so untouched deps stay lean
- **Lookups are live LMDB point queries** (`LmdbSerializer.get`: open env per call, B-tree lookup, deserialize one entry). `byPath`/`all`/`keys` are intentionally empty on `IndexedSymbolTable` — dep/JDK symbols resolve by symbol only
- Each `index.lmdb/` holds `data.mdb` + `lock.mdb`; `MapSize` is 1GB (JDK index ~570k symbols). Value format v1 (`CacheMetadata.FormatVersion`; mismatch reindexes): symbol is the key only, shortName derived at read, paths stored src-relative (`java.base/java/lang/Object.java`). JDK index ~120MB
- **Background indexing is bounded** (`indexLimiter`, 2 permits) — parsing ~90 source jars concurrently used to spike committed heap past 1GB. G1 never returns it without a Full GC: to get idle memory back automatically, set `basamake.jvmArgs` in the VS Code extension to `["-XX:G1PeriodicGCInterval=30000"]`
