...
- **Lookups are candidate-scoped LMDB point queries.** `LmdbSerializer.get`: open env per call, B-tree lookup, deserialize one entry. `get(symbol, candidates)` queries ONLY the current file's target jars (package pre-filtered via metadata.json) — precise across same-package collisions, no queries against unrelated targets — falling back to the global package-route map (`route`, built from registered jars' metadata) on a miss (covers the JDK, which is never a target dep). **Dep files get owning-jar candidates**: `candidatesForPath` derives the fingerprint from an extracted file's cache path (`<cacheRoot>/<fp>/src/<entry>`) and returns the owning jar (recovered from metadata.json when the jar isn't registered this session) plus the candidate context that first resolved into that jar (`jarCandidates`) — so lookups from inside scala-library 3.8.4 stay in 3.8.4 instead of falling to the route, which would pick sorted-first (2.12). JDK paths are excluded (`jdk-*`): src.zip can't round-trip `Fingerprint.fromJarPath`, and the JDK is the only `java.*` registerer, so the route resolves them correctly already. **Route fallback ordering**: recently-hit fingerprints first (recency LRU, bounded, cleared on overflow), then higher versions first (parsed from the fingerprint name), then lexicographic tie-break — an old scala-library must never beat a new one, and the jar the user is actually browsing wins over version. `byPath`/`all`/`keys` are intentionally empty on `IndexedSymbolTable` — dep/JDK symbols resolve by symbol only
