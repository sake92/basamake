...
`SymbolTable` is a workspace-scoped interface (`get`/`byPath`/`add`/`removeByPath`/`keys`/`all`), implemented by `InMemorySymbolTable` (`ConcurrentHashMap[String, SymbolDefinition]` keyed by SemanticDB-style symbol; thread-safe for concurrent reads during LSP requests). Dependency/JDK symbols live in `IndexedSymbolTable` — a SEPARATE read-only lookup service (no shared interface). `WorkspaceIndex` holds both (`depsTable: Option[IndexedSymbolTable]`). `WorkspaceIndex` builds the resolver's table PER FILE (`resolverTableFor`): workspace symbols plus dep symbols scoped to the file's candidates. Workspace files get the JDK-scoped lookup (JDK = implicit candidate). Dep files resolve against TARGET-SCOPED candidates: the owning jar first, then the union of the dep lists of every BSP target that registered it (a target's list is its resolved classpath — `IndexedSymbolTable.registerTarget(tid, jars)`). Jars in no target stay owning-jar-only; the JDK stays an implicit candidate. Package-segment refs (import prefixes, incl. semanticdb occurrences) resolve to the package object (`pkg/package.`); Java import statements emit per-segment refs (type/static-member only — Java has no package objects).

Source-parse mode emits CANDIDATE occurrences for dep symbols (dual type/term at importees and unresolved body refs, index-0 member candidates for `IO.pure`-style calls, declared-type candidates for member calls on typed locals like `io.flatMap` → `IO#flatMap`) — plausible full symbols verified at REQUEST time by `getSymbol` against the file's real BSP candidates; request-time prefix scans (LMDB `getPrefix`) resolve true overload keys. Template parents (`extends`/`with`/`implements`) resolve for ALL def kinds (classes, traits, objects, enums, givens, package objects, Java classes/enums/records). Semanticdb-paired files gap-merge source-parsed refs at positions the compiler left uncovered (empty-symbol cross-document SUID drops). Known limitation: importees inherited through package objects/traits (e.g. `sttp.client3.basicRequest` living on trait `SttpApi`) stay a compiler-level gap in source-parse mode.
...
- **Lookups are candidate-scoped LMDB point queries; a method-shaped miss falls back to a bounded prefix scan.** `LmdbSerializer.get`: open env per call (serialized per index path — concurrent read-only Env objects on one path trip lmdbjava's reader-slot management), B-tree lookup, deserialize one entry. `get(symbol, candidates)` iterates the current file's BSP target jars (+ the JDK `src.zip` as an implicit candidate — a dependency of every target that BSP never lists): each jar is package-filtered via metadata.json (packages ALWAYS come from the real classes jar — zip folder listing — both in the sprinkled `indexed = false` state and after a full index), a matching jar without an index is indexed in the background (a cold lookup misses fast), then one exact LMDB point query per matching jar; the first hit wins. A METHOD-SHAPED exact miss (`name().`, `name(+N).`) falls back to `LmdbSerializer.getPrefix` — a bounded (32) LMDB prefix scan for the true overload keys (the compiler's overload index is unknowable at parse time — parse-time member candidates are always index-0); type/term misses stay hard misses. A miss ends the lookup with `None`. There is NO global package route, no recency/version ranking, no remembered candidate state, no fallback search — a jar outside the target's dependency set is never consulted, even if it shares the package. A jar with no metadata (no classes-jar sibling) is unfilterable: it gets indexed in the background on first lookup. A failed query logs a warning (once per fingerprint per session) and returns `None` — there is NO corruption detection/recovery: index publication is atomic (tmp + rename), so only `CacheMetadata.isValid` (format-version bump or source staleness) ever triggers a reindex; the user deletes the cache dir to rebuild manually. **Dep files get target-scoped candidates**: `candidatesForPath` derives the fingerprint from an extracted file's cache path (`<cacheRoot>/<fingerprint>/src/<entry>`) and returns the owning jar FIRST (recovered from metadata.json when the jar isn't registered this session), then the union of the dep lists of every target that registered the owning jar (`registerTarget(tid, jars)` — the target id comes from BSP `buildTargetDependencySources` or the data.json warm start; a target's list IS its resolved classpath, so refs inside the file see exactly what the compiler saw) — **and the reference resolvers parsing that file use the same target-scoped candidates** (`WorkspaceIndex.resolverTableFor`). JDK paths return nothing (`candidatesForPath`); the implicit JDK candidate in `get` covers them. `byPath`/`all`/`keys` do not exist on `IndexedSymbolTable` — dep/JDK symbols resolve by symbol only
...
- **Background indexing, bounded — no queue.** Each cold lookup spawns one background virtual thread per jar it needs (single-flight per fingerprint via `IndexedSymbolTable`'s in-progress set: a duplicate trigger is a no-op, a failed index unmarks so a later lookup retries); the lookup itself MISSES FAST — goto-def never blocks on parsing again. A global `Semaphore` caps concurrent indexes at `max(1, min(CPUs-1, 4))`: one CPU stays free for the build server, and beyond 4 jar parsing is memory-bandwidth bound, not CPU bound. `SourceJarIndexer`'s per-fingerprint `ReentrantLock` still serializes CROSS-SERVER races on the shared cache dir (parks virtual threads, no carrier pinning). The JDK uses the same background path: `ensureJdkIndexed` (started at `initialize()` — no BSP dependency; gated by config `enableJdkIndexing`, default true) indexes `src.zip` in the background, and a COLD JDK is never indexed by a lookup — a `java.*` lookup before the background index finishes is a fast transient miss
...
- Dep/JDK cache root is per-instance: `IndexedSymbolTable(cacheRoot = ...)` / `SourceJarIndexer.index/extractEntry(..., cacheRoot)`; the LSP server derives it from `.basamake/config.json` (`depsCacheRoot`, relative → workspace root; `enableJdkIndexing: false` skips eager JDK indexing). Tests use the `TestCacheRoot` trait (per-suite `./tmp/deps-cache-*` lazy val) — never write into the real home cache
